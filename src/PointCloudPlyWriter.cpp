...
bool PointCloudPlyWriter::WriteFormatPLY(const char* filename,
                                         ...
                                         bool useBinary)
{
  return WriteFormatPLY(
    filename, points, std::vector<uint32_t>(), std::vector<uint16_t>(), useBinary);
}
                                         ...
                                         bool useBinary)
{
  ...
  stream.open(filename,
              useBinary ? (std::ios_base::out | std::ios_base::binary) : std::ios_base::out);
    ...
    if (useBinary == false)
    {
      ...
      for (int i = 0; i < static_cast<int>(points.size()); i++)
      {
        ...
        if (hasColors)
        {
          ...
          stream << " " << static_cast<uint32_t>(rgba[0]) << " " << static_cast<uint32_t>(rgba[1])
